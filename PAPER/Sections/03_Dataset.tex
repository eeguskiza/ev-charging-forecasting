...
null at the 5\% level, although the rejection is marginal (the
$p$-value sits close to the 0.05 threshold and the rolling-std plot
shows variance growing with the level). Because the marginal
stationarity is paired with a strong weekly seasonal cycle that the
plain ADF does not capture, the (S)ARIMA grid we run in
Section~\ref{sec:models} fixes $d=1$ and $D=1$ at lag~7: the first
difference removes the residual level drift, and the seasonal
difference at $m=7$ removes the weekly pattern. This pair of
differencings is also what the ACF/PACF inspection
(Fig.~\ref{fig:acf}) suggests is needed before the model orders
$(p,q)(P,Q)$ can be picked.
