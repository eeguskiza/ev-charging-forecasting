...
which we confirm experimentally in Section~\ref{sec:models}.
...
(Section~\ref{sec:models}) suggest that TimesFM is a stronger choice
...
Koohfar~\cite{koohfar2023transformer}& Transformer     & Boulder & RMSE & 0.085--0.112 (norm.) \\
Yi et al.~\cite{yi2022lstm}          & LSTM            & Utah/LA & MAE  & reported \\
Wu et al.~\cite{wu2025catformer}     & CAT-Former      & Boulder & MAE  & 0.71--0.91 (norm.) \\
Kyriakopoulos~\cite{kyriakopoulos2025comparative} & Transformer & Boulder + 3 & MAE & 0.28--0.45 (norm.) \\
\midrule
\textbf{This work} & \textbf{PatchTST + LSTM ensemble} & \textbf{Boulder} & \textbf{MAE} & \textbf{25.09 kWh (raw)} \\
\bottomrule
\end{tabular}
\end{table}
