...
focused on studies that use the same Boulder dataset. \textbf{Scores
are reported in the original publication's unit and are not directly
comparable across rows}: MAPE in $[0,1]$ for Khan et al., normalised
RMSE for Koohfar, normalised MAE for Kyriakopoulos and Wu, and raw
kWh for our work. Station selections and horizons also differ row by
row; see column 4. The table is shown so the reader can place our
contribution next to the existing literature, not as a single-number
ranking.}
\label{tab:related}
