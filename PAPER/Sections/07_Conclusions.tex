...
We compared twenty models for 7-day-ahead daily forecasting of EV
...
Twenty models, one dataset, one protocol. The verdict is uncomfortable
for the loudest hype around time-series forecasting in 2026:
foundation models did not win, and the strongest classical model
(ARIMAX) sits within one kWh MAE of the best deep learning model
(LSTM). The win at the top came from combining a small
purpose-trained transformer (PatchTST) with the tuned LSTM, and from
running each PatchTST three times to average out initialisation
noise. None of those tricks are exotic. They were, however, the
things that moved the needle on this series.
